...
<i class="fas fa-moon"></i>
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-6">
      <h2 
class="text-lg font-bold text-primary">事件列表</h2>
    ...
    <div class="mb-4 relative">
      ...
      <input type="text" id="searchInput" placeholder="按名称、类型或备注搜索..."
...
class="block text-sm font-medium mb-1">到期时间 *</label>
...
if (subscription.isActive) {
          ...
          const actionsHtml = \`
            <div class="flex items-center justify-end space-x-2">
                ...
                \${statusButtonHtml}
                <button class="delete bg-transparent hover:bg-red-50 dark:hover:bg-gray-700 text-red-600 dark:text-red-400 px-2 py-1 rounded-md text-xs font-medium" 
data-id="\${subscription.id}">删除</button>
            </div>
          \`;
row.innerHTML = 
            '<td class="px-6 py-4">' + 
...
toggleIcon.classList.remove('fa-moon');
    ...
    document.getElementById('searchInput').addEventListener('input', (e) => {
      ...
      let noDataRow = null; // 用于存储“没有事件数据”的行

      for (const row of rows) {
        const cells = row.getElementsByTagName('td');
        if (cells.length > 1) { // 这是一个订阅数据行
          ...
          // 这是“加载中”或“没有事件数据”的行
          noDataRow = row; // 暂存这一行
          row.style.display = 'none'; // 搜索时先隐藏它
        }
      }
      ...
      if (!hasVisibleDataRows && searchTerm) {
        // 如果有搜索词，但没有匹配项，则显示“未找到”
        noResultsRow = tbody.insertRow();
        noResultsRow.className = 'no-search-results';
        noResultsRow.innerHTML = '<td colspan="5" class="text-center py-4 text-secondary">未找到匹配的事件</td>';
      } else if (!hasVisibleDataRows && !searchTerm) {
        // 如果没有搜索词，也没有数据行（即搜索框为空），则显示原始的“没有事件数据”
        if (noDataRow) {
          noDataRow.style.display = ''; // 显示原始的“没有事件数据”行
 ...
 <form id="configForm" class="space-y-6 p-6">
        ...
        <span class="font-medium text-sm text-primary">WXPusher</span>
            ...
            <div class="flex flex-col items-center justify-center p-3 border rounded-lg space-y-2">
                ...
                <div class="flex items-center space-x-3">
                    <input type="checkbox" id="enableXiZhi" name="enabled_notifiers" value="xizhi" class="form-checkbox 
h-5 w-5 text-indigo-600 rounded">
     ...
     <span class="font-medium text-sm text-primary">NotifyX</span>
            ...
            <div class="flex flex-col items-center justify-center p-3 border rounded-lg space-y-2">
                ...
                <div class="flex items-center space-x-3">
                    <input type="checkbox" id="enableEmail" name="enabled_notifiers" value="email" 
        ...
        <div class="p-6 space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input 
type="text" id="wxpusherAppToken" placeholder="在 WXPusher 后台获取" class="mt-1 block w-full rounded-md shadow-sm py-2 px-3 focus:outline-none">
             </div>
            <div>
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b rounded-t-lg">
             ...
             <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" id="notifyxApiKey" placeholder="从 NotifyX 平台获取的 API Key" class="mt-1 block w-full 
rounded-md shadow-sm py-2 px-3 focus:outline-none">
            ...
            <div class="flex justify-end space-x-3 px-6 py-4 border-t bg-gray-50 rounded-b-lg">
                 <button 
type="button" data-save-button class="btn-primary text-white px-4 py-2 rounded-md text-sm">保存</button>
 ...
 <div>
          ...
          <div>
            ...
            <input type="email" id="recipientEmail" placeholder="your-email@example.com" class="mt-1 block w-full 
rounded-md shadow-sm py-2 px-3 focus:outline-none">
          </div>
         </div>
        <div class="flex justify-end space-x-3 px-6 py-4 border-t bg-gray-50 rounded-b-lg">
     ...
     <div id="telegramConfigModal" 
class="fixed inset-0 bg-gray-600 bg-opacity-50 modal-container hidden flex items-center justify-center">
        <div class="modal-content rounded-lg shadow-xl max-w-lg w-full mx-4">
    ...
    <div id="barkConfigModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 modal-container hidden flex items-center justify-center">
        <div class="modal-content rounded-lg shadow-xl max-w-lg w-full mx-4">
            <div class="flex items-center justify-between px-6 py-4 border-b rounded-t-lg">
                ...
                <button 
data-close-button class="text-gray-400 hover:text-gray-600 text-2xl">&times;</button>
            </div>
            <div class="p-6 space-y-4">
                <div>
...
const now_utc = new Date();
    ...
    for (const sub of subscriptions) {
      ...
      const originalExpiryDate = new Date(sub.expiryDate); // 捕获原始到期日
      const now_utc = new Date(); //
      
      // --- 修复点 1: 逻辑顺序调整 ---
      // 先检查过期状态，并使用 *原始* 的 sub 对象生成通知
      const hasExpired = originalExpiryDate < now_utc;
      if (hasExpired && !sub.expiredNotificationSent) {
        notificationsToSend.push({ type: '已到期', sub: { ...sub }}); // 推送 *当前* 的 sub 副本
      ...
      // --- 修复点 2: 自动续订逻辑 ---
      // 检查过期后，再处理自动续订
      if (originalExpiryDate < now_utc && sub.autoRenew !== false) { //
        let newExpiryDate = new Date(originalExpiryDate);
        ...
        sub.expiryDate = newExpiryDate.toISOString(); // 更新到期日
        // 为新的周期重置标志
